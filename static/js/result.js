document.addEventListener('DOMContentLoaded', function() {
    ...
    if (copyButton) {
        copyButton.addEventListener('click', function() {
            ...
            navigator.clipboard.writeText(reportText.trim()).then(() => {
                ...
                setTimeout(() => {
                    copyButtonText.innerText = 'Salin';
        ...
        downloadButton.addEventListener('click', function() {
            ...
            setTimeout(() => {
                try {
                    ...
                    const primaryColor = '#047857'; const textColor = '#334155'; const lightTextColor = '#6b7280';
                    const margin = 15; const pageWidth = pdf.internal.pageSize.getWidth();
                    ...
                    const addHeader = () => {
                        ...
                        pdf.setFontSize(14);
                        pdf.setTextColor('#FFFFFF'); // Warna putih untuk teks header
                        pdf.setFont(undefined, 'bold');
                        pdf.text('Laporan Analisis Daun Anggur', margin, 13);
                        
                        // PERBAIKAN: Menggambar tanggal dengan warna putih di sini
                        pdf.setFontSize(9);
                        pdf.setFont(undefined, 'normal');
                        pdf.text(formattedTimestamp, pageWidth - margin, 13, { align: 'right' });
                        
                        yPosition = 30;
                    };
                    const addFooter = (pageNumber, pageCount) => {
                        pdf.setFontSize(8); pdf.setTextColor(lightTextColor); pdf.setLineWidth(0.2);
                        pdf.line(margin, 280, pageWidth - margin, 280);
                        pdf.text(`Halaman ${pageNumber} dari ${pageCount}`, pageWidth - margin, 285, { align: 'right' });
                        pdf.text('Dihasilkan oleh Sistem Analisis Daun Anggur', margin, 285);
                    };
                    const addSection = (title, content) => {
                        if (yPosition > 250) { pdf.addPage(); addHeader(); }
                        pdf.setFontSize(12); pdf.setFont(undefined, 'bold'); pdf.setTextColor(primaryColor);
                        pdf.text(title, margin, yPosition);
                        yPosition += 8;
                        pdf.setFontSize(10); pdf.setFont(undefined, 'normal'); pdf.setTextColor(textColor);
                    ...
                    const addList = (title, list, isNumbered) => {
                        ...
                        list.forEach((item, index) => {
                            if (yPosition > 270) { pdf.addPage(); addHeader(); yPosition += 2; }
                    ...
                    pdf.setFontSize(14); pdf.setFont(undefined, 'bold'); pdf.setTextColor('#FFFFFF');
                    ...
                    const fileName = `Laporan_Analisis_${safeLabel}_${date}.pdf`;
                    pdf.save(fileName);
                    ...
                    downloadButtonText.innerText = 'Unduh';
    ...
    if (probChartCanvas && RESULT_DATA.label !== 'Negative') {
        ...
        const backgroundColors = labels.map(label => { const colorKey = (DISEASE_INFO[label] || {}).color || 'zinc'; return colorMap[colorKey]; });
        const hoverBackgroundColors = labels.map(label => { const colorKey = (DISEASE_INFO[label] || {}).color || 'zinc'; return hoverColorMap[colorKey]; });
        new Chart(probChartCanvas, {
            type: 'doughnut', data: { labels: labels, datasets: [{ data: dataValues, backgroundColor: backgroundColors, hoverBackgroundColor: hoverBackgroundColors, borderColor: document.body.classList.contains('dark') ? '#1f2937' : '#ffffff', borderWidth: 4, hoverOffset: 15 }] },
            options: {
                responsive: true, cutout: '70%', animation: { duration: 0 },
                plugins: {
                    legend: { position: 'bottom', labels: { color: document.body.classList.contains('dark') ? '#d1d5db' : '#475569', padding: 20, font: { family: "'Inter', sans-serif", size: 14, weight: '500' } } },
                    tooltip: { enabled: true, backgroundColor: '#1e293b', titleFont: { size: 14, weight: 'bold' }, bodyFont: { size: 12 }, padding: 12, cornerRadius: 8, callbacks: { label: (context) => `${context.label}: ${context.parsed.toFixed(2)}%` } }
                }
            }
        });
    }
