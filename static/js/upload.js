document.addEventListener("DOMContentLoaded", function () {
  ...
  const imagePreviewFilename = document.getElementById(
    "image-preview-filename"
  );
  const imagePreviewSize = document.getElementById("image-preview-size");
  const imagePreviewDimensions = document.getElementById(
  ...
  async function getAvailableCameras() {
    try {
      ...
      switchCameraButton.classList.toggle(
        "hidden",
  ...
  function captureImage() {
    ...
    canvas
      .getContext("2d")
      .drawImage(videoFeed, 0, 0, canvas.width, canvas.height);
    canvas.toBlob(
      (blob) => {
  ...
  function handleFileSelection(file) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      showNotification(
        "Format file tidak valid. Harap pilih JPG, JPEG, atau PNG.",
        "error"
      );
      fileInput.value = "";
      return;
    ...
    if (file.size > MAX_FILE_SIZE) {
      showNotification(
        `Ukuran file melebihi batas ${formatBytes(MAX_FILE_SIZE)}.`,
        "error"
      );
      fileInput.value = "";
      return;
  ...
  uploadForm.addEventListener("submit", function (e) {
    ...
    fetch("/upload", {
      ...
      .catch((error) => {
        showNotification(
          error.error || "Gagal mengunggah. Periksa koneksi Anda.",
          "error"
        );
